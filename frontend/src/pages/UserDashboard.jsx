...
import {
    ...
    ArrowRight, Inbox, Loader2, ShieldCheck, MapPin
} from 'lucide-react';
...
const UserDashboard = () => {
    const { user, isAuthenticated, logout, token, updateProfile } = useAuth();
    ...
    const handleAddressUpdate = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setProfileStatus({ type: 'success', message: 'Address details updated successfully!' });
            } else {
    ...
    const handlePasswordChange = (e) => {
        ...
        if (passwordStatus.type) setPasswordStatus({ type: '', message: '' });
    };
    ...
    const handlePasswordSubmit = async (e) => {
        ...
        if (!passwordData.currentPassword) {
            return setPasswordStatus({ type: 'error', message: 'Current password is required' });
        }
    ...
    const handleTabChange = (tabId) => {
        ...
        // Clear all modals on tab change
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'address':
                return (
                    <div className="bg-white p-6 lg:p-10 rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div className="divide-y divide-gray-50">
                                ...
                                <div className="p-8 bg-gray-50/50 flex justify-end">
                                    <button
                                        ...
                                        className="bg-[#0b0f1a] text-white px-10 py-4 rounded-full font-bold text-[15px] hover:bg-black transition-all disabled:opacity-50"
                                    >
                                        Book Now
                                    </button>
                                </div>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex justify-between items-center px-4 py-3 rounded-xl transition-all text-[14px] font-semibold
                                                    ${isActive
                                                ? 'border border-slate-500/50 text-white bg-slate-800/20 shadow-sm shadow-slate-900/10'
